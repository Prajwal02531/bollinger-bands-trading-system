...
This script backtests the Bollinger Bands strategy on historical IDBI Bank data
...
INTERVAL         = "1d"           # Daily candles (required for 1 year)
SMA_PERIOD       = 20
STD_MULT         = 2
STOP_LOSS_PCT    = 0.02
    ...
    for i in range(len(df)):
        ...
        if position == 'Long':
            ...
            if exit_reason:
                ...
                hold_time   = str(current_time - entry_time).split('.')[0]
...
def print_report(trades, final_balance, max_drawdown):
    ...
    # Monthly summary
    print(f"  SUMMARY FOR CONFERENCE PRESENTATION")
    print(f"{'='*60}")
